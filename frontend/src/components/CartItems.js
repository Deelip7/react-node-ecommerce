      ...
      <div className='cart'>
        ...
        <div className='cart-items'>
          ...
          <Form className='cart-items__form'>
            <Form.Field onChange={getQty} control={Select} options={[...Array(items.numInStock).keys()].map((p) => ({ text: `${p + 1}`, value: `${p + 1}` }))} defaultValue={Number(items.qty)} />
            <Popup
              content='Remove From Cart'
              style={{ boxShadow: 'none', borderRadius: '0' }}
              trigger={<Button icon='remove' size='small' basic type='button' disabled={items.numInStock === 0} className='cart-items__remove' />}
            />
          </Form>
        </div>
      </div>
